...
module.exports = {
    ...
    code: async (ctx) => {
        ...
        if (!ctx.args.length) return ctx.reply(
            ...
            quote(global.tools.msg.generateCommandExample(ctx._used.prefix + ctx._used.command, "fil 4:6"))
        );
        ...
        try {
            const apiUrl = await global.tools.api.createUrl("https://alkitab.hkbppansuran.live", `/api/v1/passage/${abbr}/${chapter}`, {});
            const {
                data
